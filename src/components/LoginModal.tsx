    ...
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
      <div className="w-full max-w-md bg-slate-900 border border-amber-500/30 rounded-3xl overflow-hidden shadow-2xl relative">
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          {/* 💡 SANDBOX FRAME / SIMULATOR EXPLANATION */}
          <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-xl space-y-2 text-[11px] leading-relaxed">
            <div className="flex items-center gap-1.5 text-amber-400 font-bold">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Jak otestovat pivní pas v náhledu?</span>
            </div>
            <ul className="list-disc pl-4 text-slate-400 space-y-1">
              <li>
                <strong className="text-slate-300">Přihlášení v iframe</strong>: Google přihlášení může být blokováno ochranou prohlížeče před cizími weby v rámcích (iframes). Pro testování v tomto rámečku jsme pro vás připravili <span className="text-amber-405 font-bold">Rychlé Google přihlášení (Demo)</span> jedním kliknutím níže, nebo klasický formulář!
              </li>
              <li>
                <strong className="text-slate-300">Testování zápisu piva (Simulace GPS)</strong>: Zápis piv a návštěv lze provést jen tehdy, když se nacházíš v okruhu **200 metrů** od hospody. Pro snadné testování z domova klikni na libovolnou hospodu a v panelu detailu vpravo stiskni tlačítko <strong className="text-amber-400 font-bold">📍 Simulovat příchod</strong>. Tím se tvá pozice okamžitě přenese a zápis se ti odemkne!
              </li>
            </ul>
          </div>
          ...
          <div className="space-y-3.5 flex flex-col items-center">
            ...
            {isIframe ? (
              <button
                ...
                className="w-[320px] h-[45px] bg-slate-950 hover:bg-slate-900 border border-slate-800 hover:border-amber-500/35 rounded-xl flex items-center justify-center gap-3 shadow-md transition cursor-pointer group animate-pulse hover:animate-none"
              >
                ...
                <span className="text-xs font-bold text-slate-200 group-hover:text-amber-500 transition">
                  Přihlásit jako David (Google Demo)
                </span>
              </button>
            ) : (
              <div id="google-signin-btn-div" className="w-[320px] h-[45px] flex justify-center items-center bg-slate-950 border border-slate-800 rounded-xl overflow-hidden shadow-md font-display">
                {!isGisLoaded && (
